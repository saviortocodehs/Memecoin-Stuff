...
def load_live(chains_selected):
    ...
    # Optional Birdeye age enrichment for Solana symbols
...
if not show_all_demo and filtered.empty and not data.empty:
    ...
    age_cap = max_age if max_age > 0 else float(pd.to_numeric(data["age_days"], errors="coerce").max() or 9999)
    relaxed = data.copy()
    relaxed = relaxed[relaxed["chain"].isin(chains)]
    relaxed = relaxed[relaxed["liquidity_usd"] >= liq_p10]
    relaxed = relaxed[relaxed["volume24h_usd"] >= vol_p10]
    relaxed = relaxed[relaxed["age_days"] <= age_cap]
    filtered = relaxed
    st.info({"auto_relax_thresholds": {"liquidity_min": round(liq_p10,2), "volume_min": round(vol_p10,2), "age_max": age_cap}})
...
if ranked.empty:
    st.error("No rows to display. Lower your thresholds or show all demo rows.")
else:
    st.dataframe(
        ranked[[
...
with tab3:
    ...
    if not ranked.empty:
        st.download_button("Export current table (CSV)", ranked.to_csv(index=False).encode("utf-8"),
